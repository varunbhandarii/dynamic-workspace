...
export function activate(ctx: vscode.ExtensionContext) {
  ...
  ctx.subscriptions.push(
    vscode.commands.registerCommand("dynamicWorkspace.calibrate", () =>
      calibrator.run()
    ),

    vscode.commands.registerCommand(
      "dynamicWorkspace.selectCamera",
      async () => {
        if (!conn) return;
        try {
          ...
          const choice = await vscode.window.showQuickPick(picks, {
            placeHolder: "Select camera index",
          });
          if (!choice) return;
          ...
          if (index === -1) {
            const s = await vscode.window.showInputBox({
              prompt: "Enter camera index (0,1,2…)",
              validateInput: (v) =>
                /^\d+$/.test(v) ? undefined : "Must be a non-negative integer",
            });
            if (!s) return;
            index = Number(s);
          }
          ...
          vscode.window.showInformationMessage(
            `Dynamic Workspace: switching camera to index ${index}…`
          );
          ...
          const picks = [0, 1, 2, 3].map((i) => ({
            label: `Camera ${i}`,
            value: i,
          }));
          const choice = await vscode.window.showQuickPick(picks, {
            placeHolder: "Select camera index (fallback)",
          });
          if (choice) {
            conn.send({ cmd: "switch_camera", index: choice.value });
          }
          vscode.window.showWarningMessage(
            `Camera listing failed: ${e?.message || e}`
          );
        }
      }
    ),
  ...
  const d1 = vscode.window.onDidChangeActiveTextEditor(() =>
    applier.editorChanged()
  );
  const d2 = vscode.workspace.onDidOpenTextDocument(() =>
    applier.editorChanged()
  );
  const d3 = vscode.window.onDidChangeTextEditorVisibleRanges((e) =>
    applier.noteScroll(e)
  );

  const d4 = vscode.workspace.onDidChangeConfiguration(async (e) => {
    if (!e.affectsConfiguration("dynamicWorkspace")) return;
    if (sensorProc && !sensorProc.consumeConfigChange()) return;

    const cfg = vscode.workspace.getConfiguration("dynamicWorkspace");
    if (
      e.affectsConfiguration("dynamicWorkspace.autoStartSensor") ||
      ...
      if (auto) await sensorProc?.restart(ctx);
      else sensorProc?.stop();
    }
  });
...
function logPolicy(
  ...
  p: ReturnType<PolicyEngine["compute"]>
) {
  const flags = Object.entries(p.health.flags || {})
    .filter(([, v]) => v)
    .map(([k]) => k);
  out.appendLine(
